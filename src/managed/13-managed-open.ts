 ...
 *
 * `managed.openAndWait` fires the open server-side (Cesto signs via the managed
...
const result = await cesto.managed.openAndWait({ user: evmWalletAddress, slug, amount });
